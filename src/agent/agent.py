...
import datetime
from src.core.db import SessionLocal, User, ConstructionUpdate, Interaction, CallLog
from src.services.llm_openai import get_response
from src.services.tts import text_to_speech, get_or_create_canned
from src.services.telephony import place_call
from src.core.memory_manager import FastMemoryManager
...
async def process_user_speech(user_id: str, user_speech: str):
    ...
    try:
        ...
        if not construction:
            raise ValueError(f"No construction update found for project '{user.project}'")

        user_lower  = user_speech.lower()
        lang_value = str(getattr(user, "language", "en"))
        lang_code = "hi" if lang_value == "hi" else "en"
        site_visit_interest = bool(getattr(user, "site_visit_interest", False))
        should_hangup = False
        ...
        if any(k in user_lower for k in visit_keywords) and not site_visit_interest:
            setattr(user, "site_visit_interest", True)
            db.commit()
        ...
        from src.services.vapi_handler import build_system_prompt
