...
from src.services.llm_openai import stream_response
from src.core.db import SessionLocal, User, ConstructionUpdate, CallLog
...
async def custom_llm_chat_completions(request: Request, background_tasks: BackgroundTasks):
    ...
    lang = "hi" if lang_value == "hi" else "en"
    ...
    if any(k in user_lower for k in VISIT_KEYWORDS) and not site_visit_interest:
        ...
        try:
            ...
            if u:
                setattr(u, "site_visit_interest", True)
                db2.commit()
