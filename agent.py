...
async def trigger_outbound_call(user_id: str):
    """Initiates the outbound call sequence for the specified user."""
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.id == user_id).first()
        if not user:
            raise ValueError(f"User {user_id} not found in the persistence layer. Ensure database is properly seeded.")
        
        # Evaluate historical interaction status for dynamic greeting
        ...
        # Initiate WebRTC/PSTN payload sequence
...
async def process_user_speech(user_id: str, user_speech: str):
    """Processes incoming speech payloads from the telephony webhook."""
    db = SessionLocal()
    try:
        user = db.query(User).filter(User.id == user_id).first()
        ...
        # 1. Commit the transcribed user speech
        db.add(Interaction(user_id=user_id, role="user", content=user_speech))
        db.commit()

        # Extract potential intent for standard disconnection
        ...
        # 2. Extract conversational continuity constraint limits
